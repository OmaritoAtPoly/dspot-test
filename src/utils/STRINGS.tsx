const STRINGS = {
  general: {
    APP_TITLE: "New Year Chaos",
    HOME: "Home",
    QUEUES: "Queues",
    CARDS: "Cards",
    NOT_FOUND: "404 - You're in the wrong place",
    ACCEPT: "Accept",
    CANCEL: "Cancel",
  },
  queue: {
    QUEUE_LENGTH: "Enter event queue length between 1-9",
    TOTAL_BRIBES: "Total bribes: ",
    TOO_CHAOTIC: "It's too chaotic",
    SET_LENGTH: "Set a valid length to process",
    BAD_ARRAY: "Enter a valid number sequence",
    ALLOWED_VALUES: "123456789",
    NO_DATA: "There is not data to show",
    EDIT: "Edit this queue",
    DELETE: "Delete this queue",
    WARNING_DELETE: "Do you want to delete this queue record_?",
    WARNING_EDIT: "Here is you queue record: ",
    CURRENT_SOLUTION:"Current Solution: ", 
  },
  yup: {
    QUEUE_RANGE: "queue length should be between 1-9",
    REQUIRED: "Value required",
  },
  table: {
    QUEUE_ARRAY: "Queue Order",
    CREATED_AT: "Queue Event Date",
    QUEUE_BRIBES: "Bribes in Queue",
  },
  cards: {
    ONLINE_CARDS_SOURCE: "https://gist.githubusercontent.com/smaugho/25712886c4a7b938fdda2c41d5b0838e/raw/79d2e4f7cfef5ba01c633400dadc81219516b95b/mixed_decks_2", 
  },
};

export default STRINGS;
